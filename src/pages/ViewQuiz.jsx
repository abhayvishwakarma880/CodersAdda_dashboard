...
import { ArrowLeft, Clock, Layout, Award, Hash, Download, FileText, CheckCircle2 } from 'lucide-react';
...
function ViewQuiz() {
  ...
  const handleExportPDF = () => {
    ...
    doc.setFontSize(10);
    doc.text(`Duration: ${quiz.duration} mins  |  Total Marks: ${quiz.questions.length * quiz.points}`, pageWidth / 2, 46, { align: "center" });
...
(C) ${q.options[2]}      (D) ${q.options[3]}`;
    ...
    if (finalY < 250) {
        doc.setFontSize(10);
        doc.text("Student Name: __________________________   Score: ________", 20, finalY + 20);
    }
    ...
    <div className="w-full mx-auto pb-20 pt-4 px-4 h-full overflow-auto">
      <div className="flex items-center justify-between mb-8 max-w-5xl mx-auto">
        <div className="flex items-center gap-4">
            ...
            <div>
            <h1 className="text-2xl font-bold" style={{ color: colors.text }}>View Quiz</h1>
            <p className="text-xs font-bold opacity-40 uppercase tracking-widest">Quiz Details & Questions</p>
            </div>
        </div>
      ...
      <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="p-4 rounded border bg-black/5 flex flex-col items-center justify-center text-center gap-2" style={{ borderColor: colors.accent + '10' }}>
              ...
              <div className="p-4 rounded border bg-black/5 flex flex-col items-center justify-center text-center gap-2" style={{ borderColor: colors.accent + '10' }}>
                  ...
                  <span className="text-xs font-bold opacity-60 uppercase">Questions</span>
                  <span className="font-bold text-lg">{quiz.questions?.length || 0}</span>
              </div>
               <div className="p-4 rounded border bg-black/5 flex flex-col items-center justify-center text-center gap-2" style={{ borderColor: colors.accent + '10' }}>
               ...
               <div className="p-4 rounded border bg-black/5 flex flex-col items-center justify-center text-center gap-2" style={{ borderColor: colors.accent + '10' }}>
                  ...
                  <span className="text-xs font-bold opacity-60 uppercase">Points</span>
          ...
          <div className="p-6 rounded border shadow-sm" style={{ backgroundColor: colors.sidebar || colors.background, borderColor: colors.accent + '20' }}>
              <h3 className="text-sm font-bold uppercase tracking-wider opacity-60 mb-2">{quiz.title}</h3>
              <p className="text-sm opacity-80 leading-relaxed">{quiz.description}</p>
          ...
          <div className="p-6 rounded border shadow-sm space-y-6" style={{ backgroundColor: colors.sidebar || colors.background, borderColor: colors.accent + '20' }}>
              ...
              <div className="space-y-4">
                  {quiz.questions?.map((q, idx) => (
                      <div key={idx} className="p-4 rounded border bg-black/5 relative" style={{ borderColor: colors.accent + '10' }}>
                          <div className="flex gap-3 mb-3">
                              <span className="text-sm font-black opacity-40">Q{idx + 1}.</span>
                              ...
                              {q.options.map((opt, oid) => (
                                  ...
                                  >
                                      <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] border ${
                                          oid === q.correctOption ? 'border-green-600 bg-green-500 text-white' : 'border-black/20'
                                      }`}>
                                          {['A','B','C','D'][oid]}
                                      </span>
                                      {opt}
                                      {oid === q.correctOption && <CheckCircle2 size={14} className="ml-auto" />}
                                  </div>
                              ))}
                          </div>
                      </div>
                  ...
                  {(!quiz.questions || quiz.questions.length === 0) && (
                      <p className="text-center opacity-40 py-8 text-sm">No questions added yet.</p>
                  )}
              </div>
          </div>
      </div>
