...
function Users() {
  ...
  return (
    <div className="h-full flex flex-col p-4 md:p-6 transition-all duration-300" style={{ backgroundColor: colors.background }}>
      {/* Fixed Header Section */}
      <div className="flex-shrink-0 mb-6 sticky top-0 z-20 pb-4" style={{ backgroundColor: colors.background }}>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          <div className="flex items-center gap-3">
             ...
             <div>
                <h1 className="text-xl md:text-2xl font-bold" style={{ color: colors.text }}>User Repository</h1>
                <p className="text-[10px] md:text-xs font-semibold opacity-40 uppercase tracking-widest" style={{ color: colors.textSecondary }}>
                  Total Network: {users.length} Identities
                </p>
             </div>
          </div>
        </div>
        ...
        <div className="relative group">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 rounded-2xl outline-none border transition-all text-sm font-semibold"
            style={{ 
      ...
      <div className="flex-1 min-h-0 flex flex-col rounded-3xl border overflow-hidden shadow-sm" style={{ borderColor: colors.accent + '15', backgroundColor: colors.sidebar || colors.background }}>
        <div className="flex-1 overflow-auto">
          <table className="w-full text-left border-collapse table-auto sm:table-fixed min-w-[600px]">
            <thead className="sticky top-0 z-30" style={{ backgroundColor: colors.sidebar || colors.background }}>
              <tr style={{ borderBottom: `2px solid ${colors.accent}15`, backgroundColor: colors.sidebar || colors.background }}>
                ...
                <th className="px-4 py-4 text-[10px] font-bold uppercase tracking-widest w-16 text-center" style={{ color: colors.textSecondary }}>Profile</th>
                <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest" style={{ color: colors.textSecondary }}>Identity Details</th>
                <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest w-48" style={{ color: colors.textSecondary }}>Contact Info</th>
                <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-right sticky right-0 z-40" style={{ backgroundColor: colors.sidebar || colors.background, color: colors.textSecondary }}>Actions</th>
              </tr>
            </thead>
                ...
                <tr key={user.id} className="hover:bg-opacity-10 transition-colors" style={{ color: colors.text }}>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      <span className="text-sm font-bold truncate pr-3">{user.name}</span>
                  ...
                  <td className="px-6 py-4 text-right sticky right-0 z-20" style={{ backgroundColor: colors.sidebar || colors.background }}>
        ...
        <div className="flex-shrink-0 px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4 border-t" style={{ borderColor: colors.accent + '15', backgroundColor: colors.sidebar || colors.background }}>
           <div className="text-[10px] font-bold uppercase tracking-[3px] opacity-40" style={{ color: colors.textSecondary }}>
           ...
           <div className="flex items-center gap-3">
             <button 
               ...
               className="p-2.5 rounded-xl transition-all disabled:opacity-20 cursor-pointer shadow-sm active:scale-95 flex items-center gap-2 text-xs font-bold border"
             ...
             <button 
               ...
               className="p-2.5 rounded-xl transition-all disabled:opacity-20 cursor-pointer shadow-md active:scale-95 flex items-center gap-2 text-xs font-bold"
