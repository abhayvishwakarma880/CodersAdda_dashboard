...
function AddShort() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!formData.instructor || !formData.description || !formData.videoUrl) {
      toast.warning("Please fill all fields and upload a video");
      return;
    }
    ...
    addShort({
        ...
        videoUrl: formData.videoUrl // In real app, upload file and get URL
    });
  ...
  return (
    <div className="w-full mx-auto pb-20 pt-4 px-4 h-full overflow-auto">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold" style={{ color: colors.text }}>Add New Short</h1>
          <p className="text-xs font-bold opacity-40 uppercase tracking-widest">Upload short video content</p>
        </div>
      </div>
                ...
                >
                    {formData.videoUrl ? (
                        <>
                            ...
                            <button 
                                type="button"
                                onClick={(e) => { e.stopPropagation(); setFormData({...formData, videoUrl: '', videoFile: null}) }}
