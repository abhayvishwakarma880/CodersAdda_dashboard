...
import { ArrowLeft, Edit, CreditCard, Clock, CheckCircle2, Shield, AlertCircle, BookOpen, FileText } from 'lucide-react';
import { useTheme } from '../../context/ThemeContext';
import { useData } from '../../context/DataContext';

function ViewSubscription() {
  const { colors } = useTheme();
  const { subscriptions, courses, ebooks } = useData();
  ...
  const includedCourses = courses.filter(c => plan.courses?.includes(c.id));
  const includedEbooks = ebooks.filter(e => plan.ebooks?.includes(e.id));
  ...
  return (
    <div className="w-full mx-auto pb-20 pt-4 px-4 h-full overflow-auto">
      ...
      <div className="max-w-4xl space-y-6">
          ...
          {/* Benefits */}
          <div className="p-6 rounded border space-y-4" style={{ backgroundColor: colors.sidebar || colors.background, borderColor: colors.accent + '20' }}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {plan.benefits?.map((benefit, i) => (
                      <div key={i} className="p-3 rounded border flex gap-3 items-center shadow-sm" style={{ borderColor: colors.accent + '10', backgroundColor: colors.background }}>
            ...
            <div className="p-6 rounded border space-y-4" style={{ backgroundColor: colors.sidebar || colors.background, borderColor: colors.accent + '20' }}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {includedCourses.map(course => (
                        <div key={course.id} className="p-3 rounded border flex gap-3 items-center shadow-sm" style={{ borderColor: colors.accent + '10', backgroundColor: colors.background }}>
            ...
            <div className="p-6 rounded border space-y-4" style={{ backgroundColor: colors.sidebar || colors.background, borderColor: colors.accent + '20' }}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {includedEbooks.map(ebook => (
                        <div key={ebook.id} className="p-3 rounded border flex gap-3 items-center shadow-sm" style={{ borderColor: colors.accent + '10', backgroundColor: colors.background }}>
                            <div className="w-8 h-8 rounded flex items-center justify-center bg-red-500/10 text-red-500">
                                <FileText size={16} />
                            </div>
                            <span className="text-sm font-bold opacity-80 flex-1" style={{ color: colors.text }}>{ebook.title}</span>
                        </div>
                    ))}
                </div>
            </div>
          ...
          <div className="p-6 rounded border space-y-6 shadow-sm" style={{ backgroundColor: colors.sidebar || colors.background, borderColor: colors.accent + '20' }}>
              ...
              <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center py-3 border-b" style={{ borderColor: colors.accent + '05' }}>
                      ...
                      <span className="font-bold text-xs" style={{ color: colors.text }}>{includedCourses.length} Included</span>
                  </div>
                  <div className="flex justify-between items-center py-3">
                      <span className="text-xs font-bold opacity-40">E-Books</span>
                      <span className="font-bold text-xs" style={{ color: colors.text }}>{includedEbooks.length} Included</span>
                  </div>
              </div>
          </div>
