...
function AddQuiz() {
    ...
    reader.onload = (evt) => {
        try {
            ...
            const parsedQuestions = data.map((row) => {
                ...
                if (correctVal.toLowerCase() === 'option a' || correctVal.toLowerCase() === 'a') correctIdx = 0;
                else if (correctVal.toLowerCase() === 'option b' || correctVal.toLowerCase() === 'b') correctIdx = 1;
                else if (correctVal.toLowerCase() === 'option c' || correctVal.toLowerCase() === 'c') correctIdx = 2;
                else if (correctVal.toLowerCase() === 'option d' || correctVal.toLowerCase() === 'd') correctIdx = 3;
                else {
                    // Try matching exact text
                    const matchIdx = options.findIndex(opt => opt.trim() === correctVal);
  ...
  const handleAddQuestion = () => {
    ...
    setFormData({
        ...formData,
        questions: [...formData.questions, { ...currentQuestion, id: Date.now().toString() }]
    });
  ...
  const handleSubmit = (e) => {
    ...
    addQuiz({
        ...formData,
        totalQuestions: formData.questions.length // Auto update total
    });
  ...
  return (
    <div className="w-full mx-auto pb-20 pt-4 px-4 h-full overflow-auto">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold" style={{ color: colors.text }}>Create New Quiz</h1>
          <p className="text-xs font-bold opacity-40 uppercase tracking-widest">Setup assessment details</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-full space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <Hash size={16} className="absolute left-3 top-1/2 -translate-y-1/2 opacity-40" />
                            <input
                                type="text"
        ...
        <div className="p-8 rounded border shadow-sm space-y-6" style={{ backgroundColor: colors.sidebar || colors.background, borderColor: colors.accent + '20' }}>
             ...
             {formData.questions.length > 0 && (
                 <div className="space-y-3">
                     {formData.questions.map((q, idx) => (
                         <div key={q.id} className="p-4 rounded border bg-black/5 flex items-start gap-3 relative group" style={{ borderColor: colors.accent + '10' }}>
                             <span className="text-xs font-black opacity-40 mt-1">Q{idx + 1}.</span>
                             <div className="flex-1">
                                 <p className="text-sm font-bold mb-2">{q.question}</p>
                                 <div className="grid grid-cols-2 gap-2">
                                     {q.options.map((opt, oid) => (
                                         <div key={oid} className={`text-xs px-2 py-1 rounded border ${oid === q.correctOption ? 'bg-green-100 border-green-200 text-green-800' : 'bg-white/50 border-black/5 opacity-60'}`}>
                                             {opt}
                                         </div>
                                     ))}
                                 </div>
             ...
             <div className="p-4 rounded border-2 border-dashed" style={{ borderColor: colors.accent + '30', backgroundColor: colors.background }}>
                 <h4 className="text-xs font-bold uppercase tracking-widest mb-4 opacity-60">Add Question manually</h4>
                 <div className="space-y-4">
                     <input
                         type="text"
                         ...
                         style={{  borderColor: colors.accent + '30', color: colors.text, backgroundColor: 'transparent' }}
                     />
