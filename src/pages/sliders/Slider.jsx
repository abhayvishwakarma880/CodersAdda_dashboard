  ...
  return (
    <div className="w-full mx-auto pb-20 pt-4 px-4 h-full overflow-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: colors.text }}>Slider Management</h1>
          <p className="text-xs font-bold opacity-40 uppercase tracking-widest">Manage homepage slider images</p>
        </div>
        <button
        ...
        {filteredSliders.map((slider) => (
          ...
          >
            <div
              ...
              onClick={() => handleImagePreview(slider.image)}
            >
              <img
                src={slider.image}
                alt={slider.title}
      ...
      {filteredSliders.length === 0 && (
        <div className="text-center py-20">
          ...
          <p className="text-sm opacity-30">Add your first slider image to get started</p>
        </div>
      )}
      ...
      <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 transition-all duration-300 ${isModalOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
        <div className="absolute inset-0 bg-black/50 backdrop-blur-sm transition-all duration-300" onClick={() => setIsModalOpen(false)} />
        <div 
      ...
      {previewImage && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 p-4" onClick={() => setPreviewImage(null)}>
          <div className="relative max-w-4xl w-full">
            ...
            <img src={previewImage} alt="Preview" className="w-full h-auto rounded shadow-2xl" />
          </div>
        </div>
      )}
    </div>
