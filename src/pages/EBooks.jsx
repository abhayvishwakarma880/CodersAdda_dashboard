...
function EBooks() {
  ...
  const { 
    ebookCategories, addEbookCategory, updateEbookCategory, deleteEbookCategory,
    ebooks, deleteEbook 
  } = useData();
  ...
  const saveEditCat = (id) => {
    if (editingCatName.trim()) {
      updateEbookCategory(id, editingCatName);
  ...
  return (
    <div className="h-full w-full flex flex-col overflow-hidden" style={{ backgroundColor: colors.background }}>
      ...
      <div className="flex-shrink-0 mb-6 flex flex-col sm:flex-row gap-4 sticky top-0 z-30 pb-4" style={{ backgroundColor: colors.background }}>
        <div className="flex-1 relative group">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 opacity-30" style={{ color: colors.text }} />
          <input 
            type="text"
            placeholder={activeTab === 'categories' ? "Search categories..." : "Search E-Books..."}
            value={searchQuery}
          ...
          {activeTab === 'categories' ? (
            <table className="w-full text-left border-collapse table-auto">
              ...
              <tbody className="divide-y" style={{ divideColor: colors.accent + '08' }}>
                {filteredCategories.map((cat, idx) => (
                  <tr key={cat.id} className="hover:bg-opacity-5 transition-colors" style={{ color: colors.text }}>
                    <td className="px-4 py-4 text-xs font-bold opacity-30 text-center">{idx + 1}</td>
                      ...
                      {editingCatId === cat.id ? (
                        <div className="flex items-center gap-2">
                          <input 
                            ...
                            autoFocus
                            onKeyPress={(e) => e.key === 'Enter' && saveEditCat(cat.id)}
                          />
                          <button onClick={() => saveEditCat(cat.id)} className="p-1.5 rounded-lg text-green-500 hover:bg-green-500/10 cursor-pointer"><Check size={18} /></button>
                          <button onClick={() => setEditingCatId(null)} className="p-1.5 rounded-lg text-red-500 hover:bg-red-500/10 cursor-pointer"><X size={18} /></button>
                        </div>
                      ) : (
            ...
            <table className="w-full text-left border-collapse table-auto">
              <thead className="sticky top-0 z-30" style={{ backgroundColor: colors.sidebar || colors.background }}>
                <tr style={{ borderBottom: `2px solid ${colors.accent}15`, backgroundColor: colors.sidebar || colors.background }}>
                  <th className="px-4 py-4 text-[10px] font-bold uppercase tracking-widest w-16 text-center" style={{ color: colors.textSecondary }}>Sr.</th>
                  <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest" style={{ color: colors.textSecondary }}>E-Book Name</th>
                  <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest" style={{ color: colors.textSecondary }}>Author</th>
                  <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest" style={{ color: colors.textSecondary }}>Category</th>
                  <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest" style={{ color: colors.textSecondary }}>Status</th>
                  <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-right" style={{ color: colors.textSecondary }}>Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y" style={{ divideColor: colors.accent + '08' }}>
                {filteredEbooks.map((book, idx) => (
                  <tr key={book.id} className="hover:bg-opacity-5 transition-colors" style={{ color: colors.text }}>
                    <td className="px-4 py-4 text-xs font-bold opacity-30 text-center">{idx + 1}</td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="p-2 rounded-lg" style={{ backgroundColor: '#ef444410', color: '#ef4444' }}>
                          <FileText size={18} />
                        </div>
                        <div className="flex flex-col">
                          <span className="text-sm font-bold truncate max-w-[150px]">{book.title}</span>
                          <span className="text-[10px] opacity-40 font-bold uppercase tracking-wider">{book.fileSize || '2.4 MB'}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-semibold opacity-70">{book.author || 'Unknown'}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className="px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider" style={{ backgroundColor: colors.primary + '15', color: colors.primary }}>
                        {book.category}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button 
                          onClick={() => navigate(`/dashboard/ebooks/view/${book.id}`)}
                          className="p-2 rounded cursor-pointer transition-all hover:bg-opacity-20"
                        ...
                        <button 
                          onClick={() => navigate(`/dashboard/ebooks/edit/${book.id}`)}
                          className="p-2 rounded cursor-pointer transition-all hover:bg-opacity-20"
                ...
                {filteredEbooks.length === 0 && (
                  <tr>
                    <td colSpan="5" className="py-20 text-center opacity-30">
                      ...
                      <p className="text-sm font-bold uppercase tracking-widest">No E-Books Found</p>
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="relative w-full max-w-md rounded p-8 shadow-2xl border" style={{ backgroundColor: colors.sidebar || colors.background, borderColor: colors.accent + '20' }}>
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <label className="text-[10px] font-bold uppercase tracking-widest opacity-50" style={{ color: colors.text }}>Category Name</label>
                <input 
                  type="text"
                  value={newCatName}
