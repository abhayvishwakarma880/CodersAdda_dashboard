...
import { ArrowLeft, Play, Info, Layout, Clock, Video, Monitor, Lock, CheckCircle, ChevronRight, FileText } from 'lucide-react';
         ...
         <div className="flex items-center gap-6 pr-4">
            ...
            {lecture.isLocked ? (
                <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 text-amber-600 border border-amber-500/20">
                    <Lock size={12} />
                    <span className="text-[10px] font-black uppercase tracking-wider">Premium Lecture</span>
                </div>
            ) : (
                <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-green-500/10 text-green-600 border border-green-500/20">
                    <CheckCircle size={12} />
                    <span className="text-[10px] font-black uppercase tracking-wider">Free Preview</span>
                </div>
            )}
         </div>
      </div>
         ...
         <div className="max-w-5xl mx-auto">
            ...
            <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-8 pb-20">
                <div className="md:col-span-2 space-y-8">
                    <div>
                        <h3 className="text-sm font-black uppercase tracking-[2px] opacity-40 mb-4">Lecture Description</h3>
                        <p className="text-sm font-semibold opacity-60 leading-relaxed whitespace-pre-wrap">
                            {lecture.description || `This lecture covers essential concepts within the ${course.title} series. Ensure you have completed previous units before proceeding for better understanding.`}
                        </p>
                    </div>

                    <div className="p-8 rounded-3xl border border-dashed" style={{ borderColor: colors.accent + '20' }}>
                        <Info size={24} className="mb-4 opacity-20" />
                        <h4 className="text-sm font-bold mb-4">Instructor's Note</h4>
                        <p className="text-xs font-medium opacity-50 leading-relaxed">
                            Watch the video carefully and follow along with the source code provided in the course resources section. 
                            If you encounter any issues, feel free to reach out via the discussion forum.
                        </p>
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    {lecture.pdfUrl && (
                        <div className="p-6 rounded-3xl border" style={{ backgroundColor: colors.sidebar || colors.background, borderColor: colors.accent + '15' }}>
                            <h3 className="text-xs font-black uppercase tracking-widest opacity-40 mb-4">Lecture Resources</h3>
                            <a 
                                href={lecture.pdfUrl} 
                                target="_blank" 
                            ...
                            >
                                ...
                                <div className="min-w-0">
                                    <p className="text-xs font-bold truncate" style={{ color: colors.text }}>{lecture.pdfFileName || 'Download Notes'}</p>
                                    <p className="text-[9px] font-bold opacity-40 uppercase tracking-widest">PDF Document</p>
                                </div>
                            </a>
                        </div>
                    )}
                    ...
                    <div className="p-6 rounded-3xl border" style={{ backgroundColor: colors.sidebar || colors.background, borderColor: colors.accent + '15' }}>
                        <h3 className="text-xs font-black uppercase tracking-widest opacity-40 mb-6">Course Tech Stack</h3>
                        <div className="flex flex-wrap gap-2">
                            {course.technology?.split(',').map((tech, i) => (
                                <span key={i} className="px-3 py-1.5 rounded-xl border text-[10px] font-black uppercase tracking-wider" style={{ borderColor: colors.accent + '20', color: colors.primary }}>
                                    {tech.trim()}
                                </span>
                            ))}
                        </div>
