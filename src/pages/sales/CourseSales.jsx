...
function CourseSales() {
  ...
  const pieChartOptions = {
    ...
    series: [{
      ...
      colorByPoint: true,
      data: [
        { name: 'Web Development', y: 45, color: '#0ea5e9' },
        { name: 'App Development', y: 25, color: '#6366f1' },
        { name: 'Data Science', y: 15, color: '#f59e0b' },
        { name: 'UI/UX Design', y: 15, color: '#10b981' }
      ]
    }],
    credits: { enabled: false }
  };
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {stats.map((stat, i) => (
          <div key={i} className="p-6 rounded-xl border shadow-sm transition-all hover:scale-[1.02]" style={{ backgroundColor: colors.sidebar, borderColor: colors.accent + '20' }}>
            ...
            <p className="text-sm font-bold opacity-60 uppercase tracking-widest" style={{color: colors.text}}>{stat.title}</p>
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        <div className="p-6 rounded-xl border shadow-sm flex flex-col items-center" style={{ backgroundColor: colors.sidebar, borderColor: colors.accent + '20' }}>
          <h3 className="font-bold text-lg mb-6 w-full text-left" style={{ color: colors.text }}>Course Distribution by Category</h3>
          ...
          <table className="w-full text-left border-collapse min-w-[600px]">
            ...
            <tbody className="text-sm">
              {successTransactions.map(txn => (
                <tr key={txn.id} className="border-b last:border-0 hover:bg-black/5 transition-colors" style={{ borderColor: colors.accent + '05', color: colors.text }}>
