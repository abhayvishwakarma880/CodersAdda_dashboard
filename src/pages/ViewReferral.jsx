...
import { ArrowLeft, Mail, Phone, GraduationCap, Share2, Copy, Check, School, BookOpen } from 'lucide-react';
...
function ViewReferral() {
  ...
  const valueStyle = { color: colors.text, fontSize: '16px', fontWeight: '600' };
  ...
  return (
    <div className="w-full  mx-auto pb-20 pt-4 px-4 h-full overflow-auto">
      <div className="flex items-center gap-4 mb-8 max-w-6xl mx-auto">
        ...
        <div>
          <h1 className="text-2xl font-bold" style={{ color: colors.text }}>Referral Details</h1>
          <p className="text-xs font-bold opacity-40 uppercase tracking-widest">User information</p>
        </div>
      </div>

      <div className="max-w-4xl mx-auto space-y-6">
          <div className="p-8 rounded border shadow-sm relative overflow-hidden" style={{ backgroundColor: colors.sidebar || colors.background, borderColor: colors.accent + '20' }}>
               ...
               <div className="flex items-center gap-4 mb-8 relative z-10">
                    ...
                    <div>
                        ...
                        <span className="text-xs font-bold opacity-50 uppercase tracking-wider">Referral User</span>
                    </div>
               </div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                   <div>
                       <div className="mb-6">
                           <p style={labelStyle} className="flex items-center gap-1"><Mail size={12} /> Email Address</p>
                           <p style={valueStyle}>{referral.email}</p>
                       </div>
                       <div>
                           <p style={labelStyle} className="flex items-center gap-1"><Phone size={12} /> Phone Number</p>
                           <p style={valueStyle}>{referral.phone}</p>
                       </div>
                   </div>
                   <div>
                       <div className="mb-6">
                           <p style={labelStyle} className="flex items-center gap-1"><School size={12} /> College</p>
                           <p style={valueStyle}>{referral.college}</p>
                       </div>
                       <div>
                           <p style={labelStyle} className="flex items-center gap-1"><BookOpen size={12} /> Course / Stream</p>
                           <p style={valueStyle}>{referral.course}</p>
                       </div>
                   </div>
               </div>

               <div className="mt-8 pt-6 border-t relative z-10" style={{ borderColor: colors.accent + '10' }}>
                    ...
                    >
                        <span className="font-mono text-xl font-bold tracking-wider">{referral.referralCode}</span>
                        {copied ? (
