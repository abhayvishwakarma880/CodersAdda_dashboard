...
import { Plus, Eye, Edit, Trash2, Search, Filter, Globe, Grid, List, MoreVertical } from 'lucide-react';
...
function WebsiteCourses() {
  ...
  const { websiteCourses, deleteWebsiteCourse, updateWebsiteCourse } = useWebsiteData();
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState('');
  const [filterType, setFilterType] = useState('All');
  const [viewMode, setViewMode] = useState('grid'); // 'grid' or 'list'

  const filteredCourses = websiteCourses.filter(course => {
    const matchesSearch = course.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         course.technology.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         course.instructor.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesFilter = filterType === 'All' || course.courseType === filterType;
  ...
  const handleStatusToggle = (id, currentStatus) => {
    ...
    updateWebsiteCourse(id, { status: newStatus });
      ...
      <div className="flex-1 overflow-auto">
        {filteredCourses.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-64">
            <Globe size={48} style={{ color: colors.textSecondary, opacity: 0.5 }} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredCourses.map((course) => (
              ...
              >
                ...
                <div className="relative h-48 overflow-hidden">
                  ...
                  <div className="absolute top-3 left-3 flex gap-2">
                    ...
                    <span
                      className="px-2 py-1 rounded text-xs font-bold text-white"
                      style={{
                        backgroundColor: course.badge === 'popular' ? '#3B82F6' : 
                                       course.badge === 'trending' ? '#10B981' : 
                                       course.badge === 'top' ? '#F59E0B' : '#6B7280'
                      }}
                    >
                ...
                {/* Course Content */}
                <div className="p-4">
                  <h3 className="font-bold text-lg mb-2 line-clamp-2" style={{ color: colors.text }}>
                    {course.title}
                  </h3>
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => navigate(`/dashboard/website/courses/view/${course.id}`)}
                    ...
                    <button
                      onClick={() => navigate(`/dashboard/website/courses/edit/${course.id}`)}
                      className="flex-1 flex items-center justify-center gap-1 py-2 px-3 rounded text-xs font-semibold transition-all"
                      style={{
