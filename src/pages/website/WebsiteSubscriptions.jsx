...
import { Plus, Eye, Edit, Trash2, Search, Grid, List, CreditCard } from 'lucide-react';
...
function WebsiteSubscriptions() {
  ...
  const { websiteSubscriptions, deleteWebsiteSubscription, updateWebsiteSubscription } = useWebsiteData();
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState('');
  const [viewMode, setViewMode] = useState('grid');

  const filteredSubscriptions = websiteSubscriptions.filter(subscription =>
  ...
  const handleStatusToggle = (id, currentStatus) => {
    ...
    updateWebsiteSubscription(id, { status: newStatus });
      ...
      <div className="flex-1 overflow-auto">
        {filteredSubscriptions.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-64">
            <CreditCard size={48} style={{ color: colors.textSecondary, opacity: 0.5 }} />
            ...
            {filteredSubscriptions.map((subscription) => (
              ...
              >
                ...
                <div className="text-center mb-4">
                  <h3 className="text-xl font-bold capitalize mb-2" style={{ color: colors.text }}>
                ...
                <div className="mb-4">
                  <h4 className="font-semibold mb-2" style={{ color: colors.text }}>Benefits:</h4>
                  <ul className="text-sm space-y-1" style={{ color: colors.textSecondary }}>
                    {subscription.benefits.split(', ').map((benefit, index) => (
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => navigate(`/dashboard/website/subscriptions/view/${subscription.id}`)}
                  ...
                  <button
                    onClick={() => navigate(`/dashboard/website/subscriptions/edit/${subscription.id}`)}
                    className="flex-1 flex items-center justify-center gap-1 py-2 px-3 rounded text-xs font-semibold transition-all"
                    style={{
                  ...
                  <button
                    onClick={() => handleDelete(subscription.id, subscription.planType)}
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          <h3 className="font-bold text-lg capitalize" style={{ color: colors.text }}>
                            {subscription.planType} Plan
                          </h3>
                          <span
                            className="px-2 py-1 rounded text-xs font-bold text-white"
