...
import { ArrowLeft, Edit, Trash2, CreditCard, Clock, DollarSign, Award } from 'lucide-react';
...
function ViewWebsiteSubscription() {
  ...
  const { websiteSubscriptions, deleteWebsiteSubscription } = useWebsiteData();

  const subscription = websiteSubscriptions.find(s => s.id === id);

  if (!subscription) {
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-lg" style={{ color: colors.textSecondary }}>Subscription not found</p>
      </div>
    );
  }
    ...
    <div className="w-full h-full flex flex-col overflow-hidden">
      <div className="flex-shrink-0 flex items-center justify-between mb-6">
        ...
        <div className="flex gap-3">
          <button
            onClick={() => navigate(`/dashboard/website/subscriptions/edit/${id}`)}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-1">
            ...
            >
              ...
              <h3 className="text-2xl font-bold capitalize mb-2" style={{ color: colors.text }}>
            ...
            >
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-start gap-3">
                  <Award size={20} style={{ color: colors.primary }} className="mt-1" />
                  <div>
                  ...
                  >
                    <ul className="space-y-2">
                      {subscription.benefits.split(', ').map((benefit, index) => (
                        <li key={index} className="flex items-start" style={{ color: colors.text }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8">
                ...
                <div
                  className="p-4 rounded-lg text-center"
                  style={{ backgroundColor: subscription.status === 'Active' ? '#22C55E20' : '#EF444420' }}
                >
                  <div className="text-2xl font-bold" style={{ color: subscription.status === 'Active' ? '#22C55E' : '#EF4444' }}>
                    {subscription.status}
                  </div>
                  <div className="text-sm font-semibold" style={{ color: colors.textSecondary }}>
                    Status
