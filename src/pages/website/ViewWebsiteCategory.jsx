...
function ViewWebsiteCategory() {
  ...
  const { websiteCategories, deleteWebsiteCategory } = useWebsiteData();

  const category = websiteCategories.find(c => c.id === id);

  if (!category) {
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-lg" style={{ color: colors.textSecondary }}>Category not found</p>
      </div>
    );
  }
            ...
            >
              ...
              <div className="space-y-6">
                <div className="flex items-start gap-3">
                  <Globe size={20} style={{ color: colors.primary }} className="mt-1" />
                ...
                <div className="flex items-start gap-3">
                  <FileText size={20} style={{ color: colors.primary }} className="mt-1" />
                  <div className="flex-1">
                    <p className="text-sm font-semibold mb-2" style={{ color: colors.textSecondary }}>
                      Description
                    </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
                ...
                <div
                  className="p-4 rounded-lg text-center"
                  style={{ backgroundColor: getStatusColor(category.status) + '20' }}
                >
                  <div className="text-xl font-bold" style={{ color: getStatusColor(category.status) }}>
                    {category.status}
                  </div>
                  <div className="text-sm font-semibold mt-1" style={{ color: colors.textSecondary }}>
                    Current Status
