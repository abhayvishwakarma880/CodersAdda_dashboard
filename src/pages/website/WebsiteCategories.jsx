...
function WebsiteCategories() {
  ...
  const { websiteCategories, deleteWebsiteCategory, updateWebsiteCategory } = useWebsiteData();
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState('');
  const [viewMode, setViewMode] = useState('grid');

  const filteredCategories = websiteCategories.filter(category =>
  ...
  const handleDelete = async (id, technology) => {
    const result = await Swal.fire({
      title: 'Are you sure?',
      text: `You want to delete "${technology}" category?`,
      icon: 'warning',
  ...
  const handleStatusToggle = (id, currentStatus) => {
    ...
    updateWebsiteCategory(id, { status: newStatus });
      ...
      <div className="flex-1 overflow-auto">
        {filteredCategories.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-64">
            <Tag size={48} style={{ color: colors.textSecondary, opacity: 0.5 }} />
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => navigate(`/dashboard/website/categories/view/${category.id}`)}
                    ...
                    <button
                      onClick={() => navigate(`/dashboard/website/categories/edit/${category.id}`)}
                      className="flex-1 flex items-center justify-center gap-1 py-2 px-3 rounded text-xs font-semibold transition-all"
                      style={{
                    ...
                    <button
                      onClick={() => handleDelete(category.id, category.technology)}
