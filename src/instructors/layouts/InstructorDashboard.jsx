...
import { Menu, Settings, X, BarChart3, BookOpen, IndianRupee, TrendingUp, LogOut, ChevronDown, GraduationCap } from 'lucide-react'
...
const InstructorDashboard = () => {
  ...
  const instructorEmail = localStorage.getItem('instructorEmail') || 'Instructor'
  ...
  return (
    <div className='flex h-screen relative overflow-hidden' style={{ backgroundColor: colors.background }}>
          ...
          {navLinks.map((link, index) => {
            ...
            return (
              <NavLink key={index} to={link.path} 
                 ...
                 onMouseEnter={(e) => {
                   if (!isActive) e.target.style.backgroundColor = colors.primary + '20'
                 }}
                 onMouseLeave={(e) => {
      ...
      {/* Overlay for mobile */}
      {sidebarOpen && (
        <div className='fixed inset-0 z-30 md:hidden' onClick={() => setSidebarOpen(false)}></div>
      )}
        ...
        <header className='h-17 border-b flex items-center px-4 md:px-6 relative'
          ...
          <div className='flex items-center space-x-2 md:space-x-4 flex-1'>
            ...
            <div className='flex flex-col'>
              <h1 className='text-sm md:text-xl font-semibold' style={{ color: colors.text }}>Instructor Panel</h1>
              <span className='text-xs md:text-sm' style={{ color: colors.textSecondary }}>{instructorEmail}</span>
            </div>
          </div>
          ...
          <div className='flex items-center space-x-2 md:space-x-4 flex-1 justify-end'>
            ...
            <div className='w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center bg-primary/10 text-primary'
      ...
      {/* Settings Modal */}
      <div className={`settings-modal fixed top-0 right-0 w-full sm:w-80 z-50 h-screen shadow-2xl transition-all duration-500 ease-in-out ${settingsOpen ? 'translate-x-0' : 'translate-x-full'}`} style={{ backgroundColor: colors.background }}>
        <div className='p-6 border-l h-full' style={{ borderColor: colors.accent + '30' }}>
          <div className='flex items-center justify-between mb-8'>
            <h2 className='text-xl font-bold' style={{ color: colors.text }}>Dashboard Settings</h2>
            <button onClick={() => setSettingsOpen(false)} style={{ color: colors.textSecondary }} className='p-2 hover:bg-black/5 rounded-full cursor-pointer transition-colors'>
              <X size={24} />
            </button>
          </div>

          <div className='mb-8'>
            <h3 className='text-sm font-bold uppercase tracking-widest opacity-40 mb-4' style={{ color: colors.text }}>Appearance</h3>
            <div className='flex items-center justify-between p-4 rounded-xl' style={{ backgroundColor: colors.accent + '05' }}>
              <span className="font-medium" style={{ color: colors.text }}>Dark Mode</span>
              <button onClick={toggleTheme} className={`w-12 cursor-pointer h-6 rounded-full relative transition-all duration-300 ${isDarkMode ? 'bg-primary' : 'bg-gray-200'}`} style={{ backgroundColor:  isDarkMode ? colors.primary : '' }}>
                <div className={`w-5 h-5 rounded-full absolute top-0.5 transition-all duration-300 ${isDarkMode ? 'translate-x-6' : 'translate-x-0.5'} bg-white shadow-sm`} style={{
          ...
          <div>
            <h3 className='text-sm font-bold uppercase tracking-widest opacity-40 mb-4' style={{ color: colors.text }}>Color Themes</h3>
            <div className='space-y-3'>
              {themeOptions.map((theme) => (
                <button key={theme.key}
                        ...
                        style={{ backgroundColor: colors.accent + '05', borderColor: currentTheme === theme.key ? colors.primary : 'transparent' }}>
                  <div className='flex items-center gap-3'>
                    <div className='w-6 h-6 rounded-full shadow-inner' style={{ backgroundColor: theme.color }}></div>
                    <span className="font-semibold" style={{ color: colors.text }}>{theme.name}</span>
                  </div>
                  {currentTheme === theme.key && <div className="w-2 h-2 rounded-full" style={{ backgroundColor: colors.primary }}></div>}
                </button>
              ))}
            </div>
          </div>
